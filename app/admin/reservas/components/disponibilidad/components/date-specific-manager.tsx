...
const DateSpecificManager = () => {
        ...
        if (targetBlocks && targetBlocks.length > 0) {
          for (const block of targetBlocks) {
            if (block.time_slots.includes("00:00")) {
              ...
              console.log(
                "No se puede desbloquear un slot de un bloqueo completo del día",
              );
              alert(
  ...
  const getDayStatus = (date: any) => {
    const dateStr = `${date.year}-${String(date.month).padStart(2, "0")}-${String(date.day).padStart(2, "0")}`;
    const hasBlocks = blockedPeriods.some(
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-6 md:grid-cols-8 gap-2">
                  {allTimeSlots.map((timeSlot) => {
                    ...
                    const slotInfo = daySlots.find(
                      (slot) => slot.time_slot === timeSlot,
                    );
                    const isAvailableByDefault =
                      slotInfo?.is_available || false;
