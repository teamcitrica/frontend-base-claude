...
export const useStudioBooking = () => {
  ...
  const getAvailableDates = async (month: number, year: number) => {
    try {
      ...
      console.log("Getting available dates for:", {
        month: month + 1,
        year,
        firstDay,
        lastDay,
  ...
  const getAvailableSlotsForDate = async (date: string) => {
    try {
      ...
      if (!existingSlots || existingSlots.length === 0) {
        ...
        console.log(
          "✅ Slots ya existen para esta fecha, usando los existentes",
        );
      }
      ...
      console.log(
        "   Activos:",
        allSlots?.filter((s) => s.is_active).length || 0,
      );
      console.log(
        "   Disponibles:",
        allSlots?.filter((s) => s.is_available).length || 0,
      );
      console.log(
        "   Activos Y Disponibles:",
        allSlots?.filter((s) => s.is_active && s.is_available).length || 0,
      );
      ...
      console.log(
        `🎯 Encontrados ${data?.length || 0} slots disponibles para ${date}:`,
