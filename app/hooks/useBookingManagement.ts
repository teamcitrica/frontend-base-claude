...
export const useBookingManagement = () => {
  ...
  const createOrGetCustomer = useCallback(
    async (
  ...
  const verifySlotAvailability = useCallback(
    async (date: string, timeSlots: string[]) => {
      try {
        ...
        if (!availabilityConfig || !availabilityConfig.is_active) {
          return {
            ...
            unavailableReasons: timeSlots.map(
              (slot) => `${slot}: Día no activo`,
            ),
          };
        }
    ...
    async (
      ...
      try {
        ...
        if (!customerResult.success || !customerResult.customer) {
          throw new Error(
            `Error creating customer: ${customerResult.error?.message || "Unknown error"}`,
          );
        }
        ...
        if (bookingData.preferredDate instanceof Date) {
          ...
          typeof bookingData.preferredDate === "object" &&
          bookingData.preferredDate &&
          "year" in bookingData.preferredDate
        ) {
        ...
        const verificationResult = await verifySlotAvailability(
          bookingDateStr,
          finalTimeSlots,
        );
  ...
  const createAdminBlock = useCallback(
    async (
      date: string,
      timeSlots: string[],
      reason: string,
  ...
  const cancelBooking = useCallback(
    async (bookingId: string): Promise<{ success: boolean; error?: any }> => {
      try {
        setIsLoading(true);
