...
export const useQuoteForm = () => {
  const { getAvailableSlotsForDate, isLoading: availabilityLoading } =
    useAvailability();
  ...
  const updateField = async (
    ...
    value: string | string[] | Date | HeroUIDate,
  ) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    ...
    if (field === "preferredDate" && value) {
      ...
      const allowMultiple = multipleResult.success
        ? multipleResult.allowed
        : true;
      ...
      if (result.success) {
        ...
        setFormData((prev) => ({
          ...prev,
          selectedTimes: prev.selectedTimes.filter((time) =>
  ...
  const selectAllTimes = () => {
    setFormData((prev) => ({
      ...prev,
      selectedTimes:
        prev.selectedTimes.length === availableTimesForSelectedDate.length
          ? []
