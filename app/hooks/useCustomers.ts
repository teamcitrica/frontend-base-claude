...
export const useCustomers = () => {
  ...
  const getCustomerStats = async (): Promise<{
    success: boolean;
    stats?: CustomerStats;
    error?: any;
  }> => {
    try {
      ...
      const { data: customersWithBookings, error: bookingsError } =
        await supabase
          .from("customers")
      ...
      const stats: CustomerStats = {
        ...
        customers_without_bookings:
          (totalCustomers || 0) - uniqueCustomersWithBookings,
      };
  ...
  const createCustomer = async (
    customerData: Omit<Customer, "id" | "created_at" | "updated_at" | "_count">,
  ) => {
    try {
      const { data, error } = await supabase
        .from("customers")
