...
export const useStudioAvailability = () => {
  ...
  const getAvailableSlotsForDate = useCallback(
    async (date: string, displayMode?: "30min" | "1hour") => {
      try {
        setIsLoading(true);
          ...
          availableSlots.forEach((slot: any) => {
            ...
            if (!processedHours.has(hour)) {
              const has00 = availableSlots.some(
                (s: any) => s.time_slot === hour,
              );
              const has30 = availableSlots.some(
                (s: any) => s.time_slot === halfHour,
              );
    ...
    async (month: number, year: number) => {
      try {
          ...
          booking.time_slots?.forEach((slot: string) => {
            if (
              ...
              for (let hour = 0; hour < 24; hour++) {
                occupiedSlotsMap
                  .get(date)
                  .add(`${String(hour).padStart(2, "0")}:00`);
                occupiedSlotsMap
                  .get(date)
                  .add(`${String(hour).padStart(2, "0")}:30`);
              }
            } else {
        ...
        for (const dateStr of potentialDates) {
          ...
          const activeSlots =
            dayConfig.time_slots?.filter((slot: any) => slot.active) || [];
          const occupiedSlots = occupiedSlotsMap.get(dateStr) || new Set();

          // Verificar si hay al menos un slot disponible
          const hasAvailableSlots = activeSlots.some(
            (slot: any) => !occupiedSlots.has(slot.slot),
          );
    ...
    async (date: string) => {
      try {
        ...
        const hasAvailableSlots =
          result.success && (result.slots?.length || 0) > 0;
