...
export const useAdminBookings = () => {
  ...
  const updateDayAvailability = async (
    dayOfWeek: number,
    isActive: boolean,
    timeSlots?: Array<{ slot: string; active: boolean }>,
  ) => {
    try {
  ...
  const toggleTimeSlot = async (
    ...
    try {
      ...
      const currentDayConfig = weeklyAvailability.find(
      ...
      const existingSlot = currentDayConfig.time_slots.find(
        (slot: any) => slot.slot === timeSlot,
      );
      ...
      if (!existingSlot) {
        ...
        updatedSlots = [
          ...currentDayConfig.time_slots,
          { slot: timeSlot, active: isActive },
        ];
      } else {
      ...
      console.log(
        "Updated slots:",
        updatedSlots.filter((s) => s.slot === timeSlot),
      );
  ...
  const createBlockedPeriod = async (
    startDate: string,
    endDate: string,
    reason?: string,
  ) => {
  ...
  const getMonthStats = async (
    month: number,
    year: number,
  ): Promise<{ success: boolean; stats?: BookingStats; error?: any }> => {
    try {
      setIsLoading(true);
  ...
  const applyWeeklyPreset = async (presetConfig: any) => {
    try {
      ...
      if (presetConfig.weekdays) {
        // Lunes a Viernes (1-5)
        const slots =
          presetConfig.weekdays.active &&
          presetConfig.weekdays.start &&
          presetConfig.weekdays.end
      ...
      if (presetConfig.saturday) {
        // Sábado (6)
        const slots =
          presetConfig.saturday.active &&
          presetConfig.saturday.start &&
          presetConfig.saturday.end
            ? generateTimeSlotsForRange(
                presetConfig.saturday.start,
                presetConfig.saturday.end,
              )
      ...
      if (presetConfig.sunday) {
        // Domingo (0)
        const slots =
          presetConfig.sunday.active &&
          presetConfig.sunday.start &&
          presetConfig.sunday.end
            ? generateTimeSlotsForRange(
                presetConfig.sunday.start,
                presetConfig.sunday.end,
              )
      ...
      if (presetConfig.everyday) {
        ...
        for (let day = 0; day <= 6; day++) {
          await updateDayAvailability(
            day,
            presetConfig.everyday.active,
            generateTimeSlotsForRange(
              presetConfig.everyday.start,
              presetConfig.everyday.end,
            ),
          );
        }
      }
      ...
      if (presetConfig.weekend) {
        ...
        for (const day of weekendDays) {
          await updateDayAvailability(
            day,
            presetConfig.weekend.active,
            generateTimeSlotsForRange(
              presetConfig.weekend.start,
              presetConfig.weekend.end,
            ),
          );
        }
    ...
    try {
      ...
      if (action === "activate") {
        ...
        await createBlockedPeriod(
          startDate,
          endDate,
          "Bloqueo masivo por administrador",
        );
      }
